...
class SwinTransformerBlock(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, window_size=8):
        ...
        if c1 != c2:
            self.conv = Conv(c1, c2)
        self.linear = nn.Linear(c2, c2)  # learnable position embedding
        self.tr = nn.Sequential(*[SwinTransformerLayer(c2, num_heads, window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2) for i in range(num_layers)])
        self.c2 = c2
    ...
    def forward(self, x):
        ...
        x = self.tr(x)
        return x
##self.model = nn.Sequential(*list(model.children())[:-2]) 这行代码是去掉 GhostNet 的分类头，只保留特征提取部分。
##forward 方法返回特征图，供后续检测头使用。
...
class GhostNetBackbone(nn.Module):
    def __init__(self, pretrained=False):
        super().__init__()
        # 注意：ghostnet函数不支持pretrained参数，所以忽略该参数
        model = ghostnet()
...
class CBAM(nn.Module):
    ...
    def forward(self, x):
        ...
        sa = self.spatial_att(torch.cat([max_out, avg_out], dim=1))
        x = x * sa
        return x

class DSConv(nn.Module):
    """深度可分离卷积：先depthwise再pointwise"""
    def __init__(self, in_ch, out_ch, k=1, s=1, p=None, act=True):
        super().__init__()
        self.dconv = nn.Conv2d(in_ch, in_ch, k, s, autopad(k, p), groups=in_ch, bias=False)
        self.pconv = nn.Conv2d(in_ch, out_ch, 1, 1, 0, bias=False)
